...
const ProjectCard = ({ project, onOpenModal }: ProjectCardProps) => {
  return (
    <div className="premium-card p-6 group cursor-pointer" onClick={() => onOpenModal(project.id)}>
      ...
      <p className="text-[var(--viction-text-secondary)] text-sm mb-4 leading-relaxed line-clamp-3">
        {project.description}
      </p>
      ...
      <div className="space-y-2 mb-4">
        {project.features.slice(0, 3).map((feature, index) => (
          <div key={index} className="flex items-start text-xs">
            <Check className="text-[var(--viction-primary)] mr-2 mt-0.5 h-3 w-3 flex-shrink-0" />
            <span className="text-[var(--viction-text-secondary)]">{feature}</span>
          </div>
        ))}
        {project.features.length > 3 && (
          <div className="text-xs text-[var(--viction-text-muted)]">
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {project.touchpoints.slice(0, 4).map((touchpoint, index) => (
          <span key={index} className="text-xs px-2 py-1 bg-[var(--viction-surface-elevated)] border border-[var(--viction-border)] rounded-md text-[var(--viction-text-secondary)]">
            {touchpoint.emoji} {touchpoint.name}
          </span>
        ))}
        {project.touchpoints.length > 4 && (
          <span className="text-xs px-2 py-1 bg-[var(--viction-surface-elevated)] border border-[var(--viction-border)] rounded-md text-[var(--viction-text-muted)]">
